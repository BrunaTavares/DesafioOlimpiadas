...
gera(p(I, N, E, C, A , H)) :-
        ...
        member(C, [estadio, paodeaçucar, ipanema, pepe, copabana]),
        member(A, [hipismo, volei, atletismo, nataçao, saltos]),
...
solucao(S) :-
	...
	member(p(ID2, _, corrida, paodeaçucar, _, _), S), ID2 @< ID1+1,
        ...
        member(p(_,belga,_,_,nataçao,_),S),
